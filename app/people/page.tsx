...
import { initials, profileUrl, readStorage, writeStorage } from "@/lib/social";

export default function PeoplePage() {
  const { data: session } = useSession();
  const [query, setQuery] = useState("");
  const [sort, setSort] = useState("Popular");
  const [following, setFollowing] = useState<string[]>(() => typeof window === "undefined" ? [] : readStorage<string[]>("trustme-people-following", []));
  const people = useMemo(() => {
    const list = currentDiscoveryPeople().filter(p => `${p.name} ${p.username}`.toLowerCase().includes(query.toLowerCase()));
  ...
  const me = session?.user?.name ? session.user.name.toLowerCase().replace(/[^a-z0-9_]/g, "") : "";
  const toggle = (username: string) => { const next = following.includes(username) ? following.filter(x => x !== username) : [...following, username]; setFollowing(next); writeStorage("trustme-people-following", next); };
