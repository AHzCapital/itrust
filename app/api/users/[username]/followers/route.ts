...
export async function GET(_request: Request, { params }: Params) {
  ...
  const rows = await prisma.follow.findMany({ where: { followingId: owner.id }, orderBy: { createdAt: "desc" }, include: { follower: { select: { id: true, username: true, name: true, image: true, verified: true, bio: true } } } });
  return NextResponse.json(rows.map((row) => row.follower));
}
