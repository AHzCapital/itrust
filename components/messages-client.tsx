...
type Thread = { id: string; currentUserId: string; other: { id: string; username: string; name: string | null; image: string | null; verified: boolean } | null; messages: { id: string; body: string; createdAt: string; senderId: string; readAt: string | null }[] };

function initials(name: string) { return name.split(/\s+/).slice(0, 2).map((x) => x[0]).join("").toUpperCase() || "T"; }

export function ConversationList() {
  const [items, setItems] = useState<Conversation[]>([]); const [loading, setLoading] = useState(true);
...
export function NewConversation({ username }: { username: string }) {
  ...
  async function send() {
    const body = message.trim(); if (!body || busy) return; setBusy(true); setError("");
    try { const response = await fetch("/api/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username, message: body }) }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || "Couldn't send your message."); window.location.href = `/messages/${data.conversationId}`; }
...
export function ConversationThread({ conversationId }: { conversationId: string }) {
  const [thread, setThread] = useState<Thread | null>(null); const [message, setMessage] = useState(""); const [error, setError] = useState(""); const [busy, setBusy] = useState(false);
  async function load() { try { const response = await fetch(`/api/messages/${encodeURIComponent(conversationId)}`, { cache: "no-store" }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || "Conversation not found."); setThread(data); setError(""); } catch (err) { setError(err instanceof Error ? err.message : "Couldn't load the conversation."); } }
  useEffect(() => { load(); const timer = window.setInterval(load, 5000); return () => window.clearInterval(timer); }, [conversationId]);
  async function send() { const body = message.trim(); if (!body || busy) return; setBusy(true); setError(""); try { const response = await fetch(`/api/messages/${encodeURIComponent(conversationId)}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: body }) }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || "Couldn't send your message."); setMessage(""); await load(); } catch (err) { setError(err instanceof Error ? err.message : "Couldn't send your message."); } finally { setBusy(false); } }
  ...
  return <main className={styles.threadPage}><header className={styles.threadHeader}><Link href="/messages"><ArrowLeft size={18}/></Link><Link href={other ? `/u/${encodeURIComponent(other.username)}` : "#"} className={styles.threadPerson}><div className={styles.avatar}>{other?.image ? <img src={other.image} alt=""/> : initials(other?.name || other?.username || "T")}</div><div><strong>{other?.name || other?.username || "Member"}</strong><span>@{other?.username}</span></div></Link></header><section className={styles.messages}>{thread.messages.length === 0 ? <div className={styles.emptyInline}>Start the conversation.</div> : thread.messages.map((item) => <div key={item.id} className={`${styles.message} ${item.senderId === thread.currentUserId ? styles.mine : ""}`}><div>{item.body}</div><time>{new Date(item.createdAt).toLocaleTimeString("en", { hour: "numeric", minute: "2-digit" })}{item.senderId === thread.currentUserId && item.readAt ? <><Check size={11}/> Read</> : null}</time></div>)}</section><div className={styles.composer}><textarea value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }} placeholder="Write a message…" rows={1}/><button className={styles.send} disabled={!message.trim() || busy} onClick={send}>{busy ? "Sending…" : "Send"} <Send size={15}/></button></div>{error && <p className={styles.error}>{error}</p>}</main>;
}
