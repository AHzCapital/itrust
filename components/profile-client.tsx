...
export default function ProfileClient() {
  ...
  async function shareProfile() { const url = `${window.location.origin}${profileUrl(profile.username)}`; if (navigator.share) { await navigator.share({ title: `${profile.name || profile.username} on Trust.Me`, text: "Verified. Valuable. Yours.", url }).catch(() => undefined); } else { await navigator.clipboard.writeText(url); toast("Profile link copied"); } }
