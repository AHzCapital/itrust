...
export default function ProfileClient() {
  ...
  function saveProfile(event: FormEvent) {
    ...
    const cleanUsername = normalizeUsername(profile.username) || "member";
    const clean = { ...profile, username: cleanUsername };
    writeStorage(PROFILE_KEY, clean);
    setProfile(clean);
    window.dispatchEvent(new CustomEvent("trustme-profile-updated"));
    setEditing(false);
    toast("Profile saved");
  }
  ...
  function toggleFollow(username: string) {
    ...
    setPeopleFollowing(next); writeStorage("trustme-people-following", next); setFollowing(next); writeStorage(FOLLOWING_KEY, next);
  }
  ...
  async function copyPost(post: SocialPost) { await navigator.clipboard.writeText(`${window.location.origin}${postUrl(profile.username, post.id)}`); toast("Post link copied"); }
  ...
  function toggleBookmark(id: string) { const next = bookmarks.includes(id) ? bookmarks.filter((x) => x !== id) : [...bookmarks, id]; setBookmarks(next); writeStorage(BOOKMARKS_KEY, next); toast(next.includes(id) ? "Saved to bookmarks" : "Removed from bookmarks"); }
  function deletePost(id: string) { const next = posts.filter((post) => post.id !== id); setPosts(next); writeStorage(POSTS_KEY, next); toast("Post deleted"); }
