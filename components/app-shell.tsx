...
import { Bookmark, Compass, Home, LogOut, Mail, Menu, Settings, Store, Users, X } from "lucide-react";
...
const items = [["Home", "/", Home], ["Explore", "/explore", Compass], ["People", "/people", Users], ["Messages", "/messages", Mail], ["Assets", "/assets", Store], ["Bookmarks", "/bookmarks", Bookmark]] as const;

function buildDefaultProfile(name: string, image: string): SocialProfile { return { name, username: normalizeUsername(name) || "member", bio: "", location: "Egypt", image, createdAt: new Date().toISOString() }; }
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (!cancelled) setProfile({ name: data.name || "Member", username: data.username, bio: data.bio || "", location: data.location || "Egypt", image: data.image || "", website: data.website || undefined, createdAt: data.createdAt });
          return;
        }
      } catch { /* fall back to local bootstrap while the database is being configured */ }
    ...
    return () => { cancelled = true; window.removeEventListener("trustme-profile-updated", sync); };
  }, [session?.user, status]);
  ...
  return <div className="appFrame">
    <aside className={mobileOpen ? "appSidebar open" : "appSidebar"}>
      ...
      <div className="appSidebarBottom" ref={menuRef}>
        ...
        {userMenuOpen && <div className="userMenu"><Link href={profileHref} onClick={close}>View profile</Link><Link href="/messages" onClick={close}>Messages</Link><Link href="/settings" onClick={close}>Settings</Link><div className="userMenuTheme"><span>Appearance</span><ThemeSwitcher/></div><button onClick={() => signOut({ callbackUrl: "/" })}><LogOut size={14}/> Sign out</button></div>}
      </div>
    </aside>
    ...
    <nav className="mobileBottomNav"><Link className={pathname === "/" ? "active" : ""} href="/"><Home size={19}/><span>Home</span></Link><Link className={pathname === "/explore" ? "active" : ""} href="/explore"><Compass size={19}/><span>Explore</span></Link><Link className="mobileCreate" href={profileHref}><span>+</span></Link><Link className={pathname === "/people" ? "active" : ""} href="/people"><Users size={19}/><span>People</span></Link><Link className={pathname.startsWith("/messages") ? "active" : ""} href="/messages"><Mail size={19}/><span>Messages</span></Link></nav>
  </div>;
}
