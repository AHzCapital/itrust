...
function buildDefaultProfile(name: string, image: string): SocialProfile {
  return {
    name,
    username: normalizeUsername(name) || "member",
    bio: "",
    location: "Egypt",
    image,
  ...
  return <div className="appFrame">
    <aside className={mobileOpen ? "appSidebar open" : "appSidebar"}>
      ...
      <div className="appSidebarBottom" ref={menuRef}>
        <button className="appUser" onClick={() => setUserMenuOpen(x => !x)} aria-expanded={userMenuOpen}>
          <div className="navAvatar">{image ? <img src={image} alt=""/> : initials}</div>
          <div><strong>{name}</strong><span>@{username}</span></div>
          <span className="userChevron">•••</span>
        </button>
        {userMenuOpen && <div className="userMenu">
          <Link href={profileHref} onClick={close}>View profile</Link>
          <Link href="/settings" onClick={close}>Settings</Link>
          <div className="userMenuTheme"><span>Appearance</span><ThemeSwitcher/></div>
          <button onClick={() => signOut({ callbackUrl: "/" })}><LogOut size={14}/> Sign out</button>
        </div>}
      </div>
    </aside>
    {mobileOpen && <button className="appOverlay" aria-label="Close menu" onClick={close}/>} 
    <div className="appMain">
      <header className="mobileAppHeader"><button onClick={() => setMobileOpen(true)} aria-label="Open menu"><Menu size={20}/></button><Link className="logo" href="/">Trust<span>.</span>Me</Link><Link href={profileHref} aria-label="My profile"><div className="navAvatar">{image ? <img src={image} alt=""/> : initials}</div></Link></header>
      {children}
    </div>
    <nav className="mobileBottomNav">
      <Link className={pathname === "/" ? "active" : ""} href="/"><Home size={19}/><span>Home</span></Link>
      <Link className={pathname === "/explore" ? "active" : ""} href="/explore"><Compass size={19}/><span>Explore</span></Link>
      <Link className="mobileCreate" href={profileHref}><span>+</span></Link>
      <Link className={pathname === "/people" ? "active" : ""} href="/people"><Users size={19}/><span>People</span></Link>
      <Link className={pathname.startsWith("/u/") ? "active" : ""} href={profileHref}><div className="navAvatar">{image ? <img src={image} alt=""/> : initials}</div><span>Profile</span></Link>
    </nav>
  </div>;
}
